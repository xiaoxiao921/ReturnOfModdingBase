...
namespace ImGui
{
	...
	inline std::pair<bool, uint8_t> Hotkey_simple(const char* label, uint8_t current_hotkey, std::optional<ImVec2> size_arg)
	{
		ImGuiWindow* window = ImGui::GetCurrentWindow();
		if (window->SkipItems)
		{
			return {false, current_hotkey};
		}
		...
		if (current_hotkey != 0 && g.ActiveId != id)
		{
			strcpy_s(buf_display, key_names[current_hotkey]);
		}
		...
		return {value_changed, current_hotkey};
	}
